...
int size;
gmp_randstate_t r;
mpz_t A, K, X, M, p, tempo, Xgmp, Xmont, X2,X3;
...
*C1_limbs, *C2_limbs, *aux_limbs, *temp_limbs, *aux2_limbs, *tempo_limbs, *q_limbs, *ZA1_limbs, *q1_limbs;
mp_bitcnt_t bit;
  ...
  //  gmp_printf("limb A %Nx\n",A_limbs,nb_limbs);
    ...
    for (i = taille-1; i>=0; i--)
    {
        ...
        #pragma omp parallel
        {
            ...
            mpn_tdiv_qr(q1_limbs, Z_limbs, 0, ZA1_limbs, (nb_limbs*2), p_limbs, nb_limbs);
            // calcul de c_1(x^2+z)
        ...
        else
        {
         ...
         #pragma omp sections
         {
            ...
            {
            // z <- x^2
            mpn_sqr(ZA1_limbs,X_limbs,nb_limbs);
            mpn_tdiv_qr(q1_limbs, Z_limbs, 0, ZA1_limbs, (nb_limbs*2), p_limbs, nb_limbs);
            // calcul de c_1(a^2+z)
...
int main(int argc, char *argv[])
{
    ...
    size = atoi(argv[1]);
    mpz_inits(A,K,X,M,p,tempo,Xgmp,Xmont,X2,X3,NULL);
